...
async function getItemDetails(
  req: Request & { params: { itemId: string } },
  res: Response
): Promise<void> {
  try {
    /* Query the database */
...
async function getMenu(
  req: Request & { params: { vendorId: string } },
  res: Response
): Promise<void> {
  try {
    /* Cast the ObjectIds */
    var castedVendorId: undefined = (req.params
      .vendorId as unknown) as undefined;
    ...
    /* Send the query results */
    if (menuItems) {
      if (menuItems.items.length > 0) res.status(200).json(menuItems.items);
