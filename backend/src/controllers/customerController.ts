...
import { ORDER_AMENDMENT_TIME_WINDOW } from "../config";
...
async function amendProfileDetails(
  ...
  if (req.body.email) await body("email").isEmail().trim().escape().run(req);
  if (req.body.givenName) await body("givenName").isAlpha().trim().run(req);
  if (req.body.familyName) await body("familyName").isAlpha().trim().run(req);
  if (req.body.password)
    await body("password").isAscii().isLength({ min: 8 }).run(req);
  ...
  try {
    ...
    if (req.body.givenName) currentCustomer.givenName = req.body.givenName;

    /* Check if the customer wants to change his family name */
    if (req.body.familyName) currentCustomer.familyName = req.body.familyName;
...
async function cancelOrder(
  ...
  /* Validate the inputs */
  await param("orderId").isMongoId().run(req);
...
async function cancelOrderAmendment(
  req: Request & {
    params: { orderId: string };
  },
  res: Response
...
async function checkoutCart(req: Request, res: Response): Promise<void> {
  try {
    ...
    req.session.cart.forEach(
      (itemOrder: IItemOrder) => (cartTotal += itemOrder.subtotal)
    );
...
async function editCartItem(
  ...
  /* Validate and sanitize the inputs */
  await param("itemId").isMongoId().run(req);
  await body("quantity").isInt().toInt().run(req);
  ...
  try {
    ...
    if (
      !existingItem ||
      !menuDetails.items.some((menuItem: IMenuItem) =>
        menuItem.itemId.equals(castedItemId)
      )
    ) {
      res.status(404).send("Not Found");
      return;
    ...
    var itemOrderIndex = req.session.cart.findIndex((itemOrder: IItemOrder) =>
      itemOrder.itemId.equals(castedItemId)
    );
    if (itemOrderIndex > -1) {
      /* Update the existing item order */
      req.session.cart[itemOrderIndex].quantity += req.body.quantity;
      req.session.cart[itemOrderIndex].subtotal +=
        existingItem.price * req.body.quantity;
      ...
      if (req.body.quantity > 0) req.session.cart.push(newItemOrder);
    }
...
async function finalizeOrderAmendment(
  req: Request & {
    params: { orderId: string };
  },
  res: Response
...
async function getCart(req: Request, res: Response): Promise<void> {
  ...
  for (var itemOrder of req.session.cart) {
    ...
    const itemDetails = await Item.findById(itemOrder.itemId).select(
      "name price"
    );

    /* Manually populate the itemId field */
    if (itemDetails) populatedItemOrder.itemId = itemDetails;
...
async function initializeOrderAmendment(
  req: Request & {
    params: { orderId: string };
  },
  res: Response
  ...
  try {
    ...
    if (req.session.cart && req.session.cart.length > 0) {
      ...
      req.session.cart.forEach((itemOrder: IItemOrder) =>
        currentCustomer.cart.push(itemOrder)
      );
      await currentCustomer.save();
    }
...
async function rateOrder(
  ...
  /* Validate and sanitize the inputs */
  await param("orderId").isMongoId().run(req);
  await body("rating").isInt({ min: 0, max: 5 }).toInt().run(req);
  if (req.body.comments) await body("comments").isAscii().trim().run(req);
...
async function register(
  ...
  /* Validate and sanitize the inputs */
  await body("email").isEmail().trim().escape().run(req);
  await body("givenName").isAlpha().trim().run(req);
  await body("familyName").isAlpha().trim().run(req);
  await body("password").isAscii().isLength({ min: 8 }).run(req);
...
async function selectVendor(
  ...
  /* Validate the inputs */
  await param("vendorId").isMongoId().run(req);
